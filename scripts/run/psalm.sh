...
echo -e "${BLUE}Запуск проверки статическим анализатором Psalm${NC}"
...
do
  if [[ -z ${s} || "$i" == *"$s"* ]]; then
    ...
    ((docker-compose exec -T php-fpm ./vendor/bin/psalm)
      || (echo -e "${RED}FATAL ERROR\nОшибка при попытке проверки статическим анализатором Psalm${NC}" && exit))
  fi
done


echo -e "${GREEN}Конец проверки статическим анализатором Psalm${NC}"